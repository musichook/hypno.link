        ...
        <div class="">
          ...
          <p class="">
            <span v-if="post.author">
              <g-link :to="`${post.author.path}/`" class="">{{ titleCase(post.author.title) }}</g-link> &bull;
            </span>
    ...
    <div v-else class="pt-24">
      <div class="">
        ...
        <p class="">
          ...
          <time :datetime="post.datetime" class="">{{ formattedPublishDate }}</time>
        </p>
      </div>
    </div>
    <nav class="absolute">
      <g-link to="/" :class="[post.cover ? '' : '']" class="">&larr; Home</g-link>
    </nav>
  </header>
</template>
